...
def compute_contour(bio_obj, image):
    assert(bio_obj.subimage_labels is not None and not bio_obj.is_surface())

    if bio_obj.is_cell():
        bio_obj.subimage_regions.sort(key=lambda x: x.area, reverse=True)
    elif bio_obj.is_nanowire():
        bio_obj.subimage_regions.sort(key=lambda x: x.bbox_area, reverse=True)
    bio_obj_region_label = bio_obj.subimage_regions[0].label
    subimage_contour = bio_obj.subimage_labels == bio_obj_region_label
...
def compute_subimage_labels_and_region_data(bio_obj, image):
    ...
    bio_obj.subimage_labels = measure.label(subimage, connectivity=2)
    bio_obj.subimage_regions = measure.regionprops(bio_obj.subimage_labels)
