...
You'll need to edit `Makefile` in the `darknet` directory of this repository. If your CPU was manufactured after 2011, you should set `AVX=1`. If you installed `libomp`, set `OPENMP=1`. Then, do a find and replace across the file replacing all instance of `gcc` with `gcc-10` and all instance of `g++` with `g++-10`.
