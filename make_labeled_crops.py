...
class BoundingBox(Box):
    ...
    def overlaps(box):
        ...
        return any(box.x1 < x < box.x2 and box.y1 < y < box.y2 \
           for point in [(self.x1,self.y1), (self.x1,self.y2), (self.x2,self.y1), (self.x2,self.y2)])
        


def parse_yolo_input(label_file, img):
    ...
    for line in file.readlines():
        ...
        bounding_boxes.append(BoundingBox(*map(int, line.split())))
